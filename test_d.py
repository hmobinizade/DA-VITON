...
def get_opt():
    ...
    parser.add_argument('--test_name', type=str, default='test_condition_first_depth_50_1024', help='test name')
    parser.add_argument("--dataroot", default="data")
    parser.add_argument("--datamode", default="test")
    parser.add_argument("--data_list", default="test_pairs.txt")
    parser.add_argument("--output_dir", type=str, default="condition_first_depth_50_1024")
    ...
    parser.add_argument('--tocg_checkpoint', type=str,
                        default='checkpoints/condition_256/condition_first_depth_256/tocg_step_036350.pth',
                        help='tocg checkpoint')
    parser.add_argument('--gen_checkpoint', type=str, default='checkpoints/generator_1024/default/gen.pth',
                        help='G checkpoint')
...
def test(opt, test_loader, tocg, generator):
    ...
    depth_estimator = pipeline(task="depth-estimation", device=0)
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            if opt.cuda:
                ...
                # new
                pil_images = [Image.fromarray((img.permute(1, 2, 0).numpy() * 255).astype(np.uint8)) for img in im]
                depth_images = [depth_estimator(pil_img)['predicted_depth'] for pil_img in pil_images]
                depth_tensor = torch.stack(depth_images, dim=0).cuda()
                depth_tensor = [F.interpolate(tensor.unsqueeze(0), size=(256, 192), mode='nearest').squeeze(0) for
                                tensor in depth_tensor]
                depth_tensor = torch.stack(depth_tensor, dim=0).cuda()
            ...
            input2 = torch.cat([depth_tensor, input_parse_agnostic_down, densepose_down], 1)
