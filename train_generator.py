...
def get_opt():
    ...
    parser.add_argument('--name', type=str, default="generator_depth_256")
    ...
    parser.add_argument('--tocg_checkpoint', type=str, help='condition generator checkpoint', default='checkpoints/condition_256/condition_base_256/tocg_step_109049.pth')
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step + opt.decay_step)):
        ...
        with torch.no_grad():
            if not opt.GT:
                ...
                depth = F.interpolate(depth, size=(256, 192), mode='nearest')
                ...
                for i in range(opt.num_test_visualize):
                    grid = make_image_grid(
                        [(c_paired[i].cpu() / 2 + 0.5),
                         (cm[i].cpu()).expand(3, -1, -1),
                         ((pose.cpu()[i]+1)/2),
                         visualize_segmap(parse_agnostic.cpu(), batch=i),
                        (warped_cloth_paired[i].cpu() / 2 + 0.5),
