...
def get_opt():
    ...
    parser.add_argument('--fp16', default=True, action='store_true', help='use amp')
    # Cuda availability
    parser.add_argument('--cuda',default=True, help='cuda or cpu')

    parser.add_argument('--test_name', type=str, default='test_condition_first_depth_cloth_front_multiheadatt_8_150p_1024_u', help='test name')
    parser.add_argument("--dataroot", default="C:/hd-vton-dataset")
    parser.add_argument("--datamode", default="test")
    parser.add_argument("--data_list", default="test_pairs.txt")
    parser.add_argument("--output_dir", type=str, default="condition_first_depth_cloth_front_multiheadatt_8_150p_1024_u")
    ...
    parser.add_argument('--tocg_checkpoint', type=str, default='checkpoints/condition_256/condition_first_depth_cloth_front_256_multiheadatt_8/tocg_step_101780.pth', help='tocg checkpoint')
    parser.add_argument('--gen_checkpoint', type=str, default='checkpoints/generator_1024/default/gen.pth', help='G checkpoint')

    parser.add_argument("--tensorboard_count", type=int, default=100)
    parser.add_argument("--shuffle", action='store_true', help='shuffle input data', default=False)
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            if opt.cuda :
                ...
                pre_clothes_mask = inputs['cloth_front_mask'][opt.datasetting].cuda()
                label = inputs['parse']
                parse_agnostic = inputs['parse_agnostic']
                agnostic = inputs['agnostic'].cuda()
                clothes = inputs['cloth_front'][opt.datasetting].cuda() # target cloth
                ...
                # # input1
                # cc_paired = inputs['cloth_canny']['paired'].cuda()
                # cc_paired = torch.FloatTensor((cc_paired.detach().cpu().numpy() > 0.5).astype(float)).cuda()
            ...
            # cc_paired = F.interpolate(cc_paired, size=(256, 192), mode='bilinear')
            densepose_down = F.interpolate(densepose, size=(256, 192), mode='bilinear')
            depth = F.interpolate(depth, size=(256, 192), mode='bilinear')

            shape = pre_clothes_mask.shape
            
            # multi-task inputs
            input1 = torch.cat([clothes_down, pre_clothes_mask_down], 1)
            input2 = torch.cat([depth, input_parse_agnostic_down, densepose_down], 1)
            ...
            for i in range(shape[0]):
                grid = make_image_grid([(clothes[i].cpu() / 2 + 0.5), (pre_clothes_mask[i].cpu()).expand(3, -1, -1), visualize_segmap(parse_agnostic.cpu(), batch=i), ((densepose.cpu()[i]+1)/2),
                                        ...
                                        (im[i].cpu()/2 +0.5), (output[i].cpu()/2 +0.5)],
                                        nrow=4)
...
def main():
    ...
    input2_nc = opt.semantic_nc + 3 + 1  # parse_agnostic + densepose
