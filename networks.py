...
class MultiHeadAttention(nn.Module):
    def __init__(self, in_dim, num_heads):
        ...
        assert in_dim % num_heads == 0, "in_dim باید بر num_heads بخش‌پذیر باشد!"
        ...
        self.out_conv = nn.Linear(in_dim, in_dim)
...
class PatternAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        Q = self.query_conv(x).view(B, C, N).transpose(1, 2)  # (B, N, C)
        K = self.key_conv(x).view(B, C, N).transpose(1, 2)  # (B, N, C)
        V = self.value_conv(x).view(B, C, N).transpose(1, 2)  # (B, N, C)
...
class ConditionGenerator(nn.Module):
    def __init__(self, opt, input1_nc, input2_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.ClothEncoder = nn.Sequential(
            ...
            ResBlock(ngf * 2, ngf * 4, norm_layer=norm_layer, scale='down', att="MultiHeadAttention"),  # 32
        ...
        self.PoseEncoder = nn.Sequential(
            ...
            ResBlock(ngf * 2, ngf * 4, norm_layer=norm_layer, scale='down', att="MultiHeadAttention"),
            ResBlock(ngf * 4, ngf * 4, norm_layer=norm_layer, scale='down'),
            ResBlock(ngf * 4, ngf * 4, norm_layer=norm_layer, scale='down')
        )
        
        self.conv = ResBlock(ngf * 4, ngf * 8, norm_layer=norm_layer, scale='same', att="MultiHeadAttention")
...
class ResBlock(nn.Module):
    def __init__(self, in_nc, out_nc, scale='down', norm_layer=nn.BatchNorm2d, att=False, num_heads=8):
        ...
        self.MultiHeadAttentionBlock = nn.Sequential(
            nn.Conv2d(out_nc, out_nc, kernel_size=3, stride=1, padding=1, bias=use_bias),
            norm_layer(out_nc),
            nn.ReLU(inplace=True),
            MultiHeadAttention(out_nc, num_heads),  # Attention layer added here
            nn.Conv2d(out_nc, out_nc, kernel_size=3, stride=1, padding=1, bias=use_bias),
            norm_layer(out_nc)
        )

        self.PatternAttentionBlock = nn.Sequential(
            nn.Conv2d(out_nc, out_nc, kernel_size=3, stride=1, padding=1, bias=use_bias),
            norm_layer(out_nc),
            nn.ReLU(inplace=True),
            PatternAttention(out_nc, num_heads),  #   PatternAttention
            nn.Conv2d(out_nc, out_nc, kernel_size=3, stride=1, padding=1, bias=use_bias),
            norm_layer(out_nc)
        )
    ...
    def forward(self, x):
        ...
        if self.att == "MultiHeadAttention":
            return self.relu(residual + self.MultiHeadAttentionBlock(residual))
        elif self.att == "PatternAttention":
            return self.relu(residual + self.PatternAttentionBlock(residual))
