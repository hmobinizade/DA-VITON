...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in ['paired']:
            ...
            cf[key] = Image.open(osp.join(self.data_path, 'cloth-front', c_name[key].replace(".jpg", ".png")))
            cf[key] = transforms.Resize(self.fine_width, interpolation=0)(cf[key])

            cfm[key] = Image.open(osp.join(self.data_path, 'cloth-front-mask', c_name[key].replace(".jpg", ".png")))
            cfm[key] = transforms.Resize(self.fine_width, interpolation=0)(cfm[key])
            # --------------------------------------------------------------------------------------

            c[key] = self.transform(c[key])  # [-1,1]
            cf[key] = self.transform(cf[key])  # [-1,1]
            ...
            cfm_array = np.array(cfm[key])
            cfm_array = (cfm_array >= 128).astype(np.float32)
            cfm[key] = torch.from_numpy(cfm_array)  # [0,1]
            cfm[key].unsqueeze_(0)
        ...
        # load depth image
        im_depth_big = im_name.replace('image', 'img-depth-v2').replace('.png', '.jpg')
        im_depth_big = Image.open(osp.join(self.data_path, im_depth_big))
        im_depth_pil = transforms.Resize(self.fine_width, interpolation=0)(im_depth_big)
        im_depth = ToTensor()(im_depth_pil)
        ...
        result = {
            ...
            # intput 1 (clothfloww)
            # 'cloth':    c,          # for input
            'cloth_front':    cf,          # for input
            # 'cloth_mask':     cm,   # for input
            'cloth_front_mask':     cfm,   # for input
